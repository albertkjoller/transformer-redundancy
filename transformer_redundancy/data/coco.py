...
def get_coco_loaders(data_path_str: str, processor, batch_size=128, dset_tags: str = ['val2017'], seed: int = 0):
    DATA_PATH = Path(data_path_str)
    
    loaders = {}
    for dset_tag in dset_tags:
        # Load COCO dataset from pre-saved files
        dataset = CocoCaptions(root=DATA_PATH / dset_tag, annFile=DATA_PATH / f'annotations/captions_{dset_tag}.json')
        # Create DataLoader
        loaders["test"] = loaders["validation"] = DataLoader(COCORev(dataset, processor), batch_size=batch_size, shuffle=True)
