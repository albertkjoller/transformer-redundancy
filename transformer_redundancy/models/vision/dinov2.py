    ...
    def __register_hooks__(self, register_intermediate: bool = False):
        ...
        for layer_idx in range(self.num_layers):
            if register_intermediate:
                self.model.dinov2.encoder.layer[layer_idx].layer_scale2.register_forward_hook(self.get_features(f"layer{layer_name}"))
                self.model.dinov2.encoder.layer[layer_idx].register_forward_hook(self.get_features(f"layer{layer_name + 1}"))
                layer_name += 2
            else:
                self.model.dinov2.encoder.layer[layer_idx].register_forward_hook(self.get_features(f"layer{layer_idx}"))
